...
0. **Fase 8 — `MusicTag` → taggit facetado. EN MARCHA, esperando un despliegue.** El comando de re-etiquetado (C34a) está escrito, con 14 tests, y su ensayo en seco valida el mapa entero. Lo que falta para poder ejecutarlo es **C33, la migración de esquema**, que es el paso que querías confirmar antes. Ver la sección "Fase 8", con la decisión de secuencia expandir → migrar → contraer.
...
- [~] **C33 — Las cuatro páginas tienen manager de taggit. HECHA Y ENSAYADA; FALTA DESPLEGAR.** *2026-08-22, commit `547a547`, subido a `origin/main`.*
  ...
  - **Falta el despliegue**, que es lo único que la deja cerrada: `just deploy-production`. El clasificador de permisos de la sesión lo bloqueó, así que lo lanza el principal.
