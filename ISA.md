...
0. **Fase 8 — `MusicTag` → taggit facetado. EN MARCHA. C34b ensayada; falta desplegarla y lanzarla.** El comando de re-etiquetado (C34a) está escrito, con 14 tests, y su ensayo en seco valida el mapa entero. Lo que falta para poder ejecutarlo es **C33, la migración de esquema**, que es el paso que querías confirmar antes. Ver la sección "Fase 8", con la decisión de secuencia expandir → migrar → contraer.
...
- [~] **C34b — `--ejecutar` escribe las etiquetas en las páginas. ENSAYADA CONTRA DATOS REALES; FALTA DESPLEGAR Y LANZAR.** *2026-08-23, commit `9ed4e1f`.*
  ...
  - **Falta:** desplegar y lanzar `migrar_musictags --ejecutar` contra producción. Los dos comandos los lanza el principal — el clasificador de permisos de la sesión bloquea el despliegue.
