...
0. **Fase 8 — `MusicTag` → taggit facetado. EN MARCHA. Toca C34b: llenar el campo.** El comando de re-etiquetado (C34a) está escrito, con 14 tests, y su ensayo en seco valida el mapa entero. Lo que falta para poder ejecutarlo es **C33, la migración de esquema**, que es el paso que querías confirmar antes. Ver la sección "Fase 8", con la decisión de secuencia expandir → migrar → contraer.
...
- [ ] **C34b — `--ejecutar` escribe las etiquetas en las páginas. DESBLOQUEADA, sin empezar.** C33 ya está en producción, así que el campo existe y el comando ya no aborta. *Lo que falta: el cuerpo de `_aplicar` está escrito pero **SIN VERIFICAR** — se escribió cuando no había dónde probarlo. Antes de tocar producción hay que (a) decidir si basta `.set()` + `save()` o hace falta `save_revision()` para que las etiquetas sobrevivan al ciclo de revisiones de Wagtail, (b) tests de escritura, (c) ensayo con `--ejecutar` sobre la copia local de producción, comparando contra `backups/control_c35_antes.json`.*
