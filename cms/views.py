...
def filtered_scores_view(request):
    ...
    tag_names = request.GET.get('tags', '').split(',') if request.GET.get('tags') else []
    category_names = request.GET.get('categories', '').split(',') if request.GET.get('categories') else []
    ...
    document_tag_names = request.GET.get('document_tags', '').split(',') if request.GET.get('document_tags') else []
    ...
    if difficulty_filter:
        ...
        for score in scores:
            ...
            for block in pdf_blocks + audio_blocks + image_blocks:
                if hasattr(block, 'difficulty_level') and block.difficulty_level == difficulty_filter:
