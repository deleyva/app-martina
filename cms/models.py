...
class BookmarkBlock(StructBlock):
    ...
    page_number = CharBlock(max_length=10, help_text="Número de página o rango (ej: '5' o '5-8')")
...
class MetadataBlock(StructBlock):
    ...
    key_signature = CharBlock(max_length=20, required=False, help_text="ej: C mayor, F# menor")
    tempo = CharBlock(max_length=20, required=False, help_text="BPM o indicación de tempo")
    ...
    reference = CharBlock(max_length=200, required=False, help_text="Número de catálogo, opus, etc.")
...
class MusicTag(models.Model):
    ...
    color = models.CharField(max_length=7, default="#3B82F6", help_text="Código de color hex")
    created_at = models.DateTimeField(auto_now_add=True)
...
class MusicLibraryIndexPage(Page):
    ...
    intro = RichTextField(blank=True, help_text="Texto de introducción para la biblioteca musical")
    ...
    def get_context(self, request):
        ...
        try:
            scores = ScorePage.objects.child_of(self).live().order_by('-first_published_at')
...
class SetlistPage(Page):
    ...
    setlist_items = StreamField([
        ('score_reference', PageChooserBlock(
        ...
        ('separator', StructBlock([
            ('title', CharBlock(max_length=100, help_text="Título de sección (ej: 'Intermedio')")),
        ], icon='horizontalrule')),
    ], blank=True, use_json_field=True)
